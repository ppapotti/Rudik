...
The estimated marginal weight for a valid rule instead is equal to the actual marginal weight from Definition~\ref{def:marweight}.
...
We use the concept of \emph{frontier nodes} for a rule $r$, namely $N_f(r)$. Given a rule $r$, $N_f(r)$ contains the last visited nodes in the paths that correspond to $r_{body}$ from every example graphs covered by $r$. 
...
The function \texttt{expandFrontiers} retrieves from the KB all nodes (along with edges) at distance $1$ from frontier nodes and returns the set of all rules generated by this one hop expansion. 
