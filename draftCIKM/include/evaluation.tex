...
We carried out an extensive experimental evaluation of our approach and grouped the results in three main sub-categories: 
...
Full test results, including annotated examples and rules, are available at \url{http://***double-blind***}.
...
\begin{figure}[ht]
	...
	\vspace{-2ex}
	\caption{Accuracy for new facts identified by executing rules according to descending score on \dbpedia (no literals).}
	\label{fig:vs_amie_dbpedia}
\end{figure}

For this evaluation, we plot the total cumulative number of new unique facts (x-axis) versus the aggregated precision (y-axis) when incrementally including in the solution the rules according to their descending score. Figures~\ref{fig:vs_amie_yago} and \ref{fig:vs_amie_dbpedia} report the results
...
can be found  at \url{http://***double-blind***}.
