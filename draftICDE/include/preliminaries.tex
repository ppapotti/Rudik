...
The approximate version of the discovery problem aims to identify the rules that cover all elements in $G$ and as few as possible elements in $V$. Since we do not want overfitting rules, we do not generate in $R$ rules with constants only in the target variables.
...
the set of elements (universe) corresponds to the generation examples in $G$, the input sets are identify by the rules defined in $R$, the nonnegative weight
