...
The evaluation of negative rules is then carried out as explained before: we generate potential errors in the KB, and we manually evaluated the precision of such errors. Table~\ref{tab:vs_amie_neg} shows the results on the two KBs.
\begin{table}[t]
	\centering
...
The main goal of this set of experiments is to prove the applicability of our approach in providing Machine Learning algorithms meaningful training examples.
...
In this set of experiments we will use our negative rules on \dbpedia to generate negative examples, and we will compare the output of \deepdive trained with different set of negative examples. We used \deepdive spouse showcase example
%\footnote{\url{http://deepdive.stanford.edu/}}, where the goal is to extract mentions of married people from text articles
. \deepdive already provides some negative rules to generate negative examples (e.g., if two people appear in a sentence connected by the words \textit{brother} or \textit{sister} then they are not married). We therefore compare the output of \deepdive using our generated negative examples and the ones generated with \deepdive rules. 
