...
Figure~\ref{fig:vs_amie_yago} plots the total number of new unique predictions (x-axis) versus the aggregated precision (y-axis) on \yago. 
It is crucial to note that in \krd that we drop the conventional ranking mechanism and rather opt to use a scoring function that discovers only inherently meaningful rules with enough support that is also reflected in the high precision (85\% before the last rule, with more than 80K predictions) we obtain with a concise rule set .

 \amie outputs produces many more predictions (262K vs 102K), but with a significant lower accuracy. This is because, many good rules are preceded by meaningless ones in the ranking, and it is not clear how to set a proper $k$ in order to get the best top $k$ rules. Hence  \krd instead understands that in some cases meaningful rules do not exist, and it outputs something only when it has a strong confidence.
