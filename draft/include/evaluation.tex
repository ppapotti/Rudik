...
We carried out an extensive experimental evaluation of our rule discovery approach.
...
\begin{inparaenum}[\itshape(i)]
	...
	\item showing the applicability of rule discovery in aiding Machine Learning algorithms with representative training data;
	\item sketching the internal system settings and some KB properties.
\end{inparaenum}
...
We evaluated our approach over the latest core facts derived from several popular KBs. All the experiments were run on an iMac desktop with
an Intel quad-core i5 at 2.80GHz with 16 GB RAM. We ran \system{OpenLink Virtuoso} server with its SPARQL query endpoint on the same machine, optimized for an 8 GB available RAM.

Our method needs the two input parameters $\alpha$ and $\beta$ of Equation~\ref{eq:weight_fun} which we set to  $\alpha=0.3$ and $\beta=0.7$ for positive rules, while $\alpha=0.4$ and $\beta=0.6$ for negative rules respectively. We also use a  $maxPathLen$ parameter indicating the number of atoms in each of the rules to be discovered that is set to $3$. We justify the choices of these parameters in Section~\ref{sec:krd_int_evaluation}. 
\begin{comment}
...
We evaluate the effectiveness of \krd in its ability to discover both positive and negative rules. The set of predicates for evaluation are chosen separately in the case of positive and negative rules from each KB as follows: we pick $5$ popular predicates occurring in a majority of triples out of which the top $3$ are known to have at least one meaningful rule while there is no prior knowledge about the rules pertaining to the other 2 predicates. \krd can discover rules over the entire KB if the input is the universal set of predicates from the KB. This is further elaborated in Section~\ref{sec:krd_comparative}.

The evaluation of the rules discovered by \krd and the baseline algorithms we compare against is done on similar lines as ~\cite{galarraga2015fast} for both positive and negative rules. If a rule is universally correct, we mark all its new predictions as true. If a rule is unknown, we randomly sampled 30 new predictions likely to be violating rule head and manually checked them against the Web. 
The \emph{precision} of a rule is then computed as the ratio of true predictions out of true and false predictions. The subtle difference is that the sample set of tentatively violating predictions to be evaluated are generated differently for positive and negative rules. As an example, for the positive rule $\atom{spouse}{a}{v0} \wedge \atom{child}{v0}{b} \Rightarrow \atom{child}{a}{b}$, we retrieve all those pairs  $(a,b)$ such that $b$ is the child of $v0$ and $v0$ is the spouse of $a$ but $b$ is not the child of $a$. In the case of the negative rule, $\atom{child}{a}{b} \Rightarrow \neg \atom{spouse}{a}{b}$, we retrieve all those pairs $(a,b)$ such that $b$ is \texttt{child} of $a$ and $a$ is \texttt{spouse} with $b$.
\begin{comment}
...
The first set of experiments aims at evaluating the accuracy of discovered rules over the 3 most popular and widely used KBs: \dbpedia~\cite{bizer2009dbpedia}, \yago~\cite{suchanek2007yago}, and \wikidata~\cite{vrandevcic2014wikidata}. 
...
Table~\ref{tab:krdDatasetDescr} shows the characteristics of the 3 KBs.
...
%As the figure shows, 
The size of the KB holds special significance as loading the entire KB in memory is not feasible unless we either have huge memory at our disposal~\cite{Chen:2016,DBLP:conf/sigmod/FaridRIHC16},  or we artificially shrink the KB by eliminating all the literals~\cite{galarraga2015fast}. We propose an approach that is disk-based where only a small portion of the KB is loaded in memory, such that we can discover rules on any sized KB with limited resources while retaining the literals.
...
Our first observation is that the more accurate is the KB, the better is the quality of induced rules. \wikidata contains very few errors, since it is manually curated. 
%and every triple is manually checked by different individuals before being inserted
 \dbpedia and \yago instead are automatically generated by extracting information from the Web, hence their quality is significantly lower. 
\begin{comment}
...
\end{comment}
Predicates like \texttt{academicAdvisor}, \texttt{child}, and \texttt{spouse} have a precision above $95\%$ in all of the KBs, and final precision values are brought down by few predicates like \texttt{founder} where meaningful rules probably do not exist at all.

The running time expended in rule discovery is influenced by various factors such as the size of the KB both in terms of the number of triples and the number of predicates. \krd is slower in \wikidata which is the biggest KB. The more predicates we have in the KB, the more alternative paths we observe while traversing the graph thus resulting in a bigger search space. The second relevant aspect is the target predicate involved. We noticed that some kind of entities have a huge number of outgoing and incoming edges (``\textit{United States}'' in \wikidata is connected to more than $600$K entities). When the generation set includes such entities, the navigation of the graph is slower as we need to traverse a higher number of edges. 
%This is what happens in \yago, where the most popular predicates are \texttt{isLeaderOf} and \texttt{exports}. 
Eventually the $maxPathLen$ parameter also has a big say in the final running time. The longer the rule, the bigger is the search space. 
...
\begin{table}[b]
	\centering
	\caption{Negative Rule Accuracy.}
...
%For example, we identify the same rule that two people with same gender cannot be married both in \yago and \wikidata. Such a rule has a 94\% precision in \yago, while the accuracy in \wikidata is 57\%.
 \yago is the KB with the highest number of errors. As an example, there are $9,057$ cases in the online \yago where a child is born before her parent. 
...
Last, discovering negative rules is faster than discovering positive rules because the time consuming validation queries are faster upon positive validation sets than over positive validation sets. The issue of negative validation sets contributing to complex counter examples while evaluating in standard SPARQL engines has also been touched upon in Section~\ref{sec:ex_generation}.
\begin{comment}
 This is mostly due to the time we spend executing validation coverage queries. 
%Whenever we discover a rule that respect our language bias (Section~\ref{sec:krd_language}), we execute the body of the rule against the KB with a SPARQL query to compute its coverage over the validation set. 
These queries are faster for negative rules since the validation set is just all the entities connected by the target predicate, whereas in the positive case the validation set corresponds to counter examples described in Section~\ref{sec:ex_generation}, which are usually more complex to evaluate for standard SPARQL engines.
\end{comment}
...
We compared the performance of our rule discovery method against \amie~\cite{galarraga2015fast}, the state-of-the-art positive Horn Rule discovery system for KBs.

\amie  loads the entire KB into memory, and then discovers positive rules for every predicate in the KB. 
...
%Differently from us, 
\amie outputs all possible rules that exceed a given threshold and ranks them according to the coverage function.

Given the in-memory implementation, \amie cannot handle large KBs. We tried to run it on the KBs of Table~\ref{tab:krdDatasetDescr}, but the system goes quickly out of memory. Thus we downloaded and used the modified versions of \yago and \dbpedia from \amie which are devoid of literals and \texttt{rdf:type} facts .
...
\amie outputs a huge amount of rules 
...
Figures~\ref{fig:vs_amie_yago} and ~\ref{fig:vs_amie_dbpedia} plot the total number of new unique predictions (x-axis) versus the aggregated precision (y-axis) on \yago and \dbpedia respectively. 
It is crucial to note that in \krd that we drop the conventional ranking mechanism and rather opt to use a scoring function that discovers only inherently meaningful rules with enough support that is also reflected in the high precision we obtain with a concise rule set in both \yago and \dbpedia .

 \amie outputs produces many more predictions, but with a significant lower accuracy. This is because, many good rules are preceded by meaningless ones in the ranking, and it is not clear how to set a proper $k$ in order to get the best top $k$ rules. Hence  \krd instead understands that in some cases meaningful rules do not exist, and it outputs a rule only when it has a strong confidence.
...
As a consequence, \krd outputs just 11 rules for 8 target predicates on the entire \yago -- for the remaining predicates \krd does not find any rule with enough support.  In fact if we limit the output of \amie to the best 11 rules (same output of our approach), the final accuracy is still 29\% below our approach, with just 10K more predictions.
...
%For example, we added a \texttt{notSpouse} predicate connecting each pair of people who are not married according to our negative examples generation technique. We then run \amie on these new created predicates. 
...
\begin{table}[t]
	...
	\end{small}
\end{table}
Table~\ref{tab:vs_amie_neg} shows that \krd outperforms \amie in both cases with a precision gain of almost 20\%.
...
We report here the running time of \amie compared to our approach. Note that numbers are different from~\cite{galarraga2015fast}, where \amie was run on a 48 GB RAM server. \amie could finish the computation only on \yago 2, while for other KBs it got stuck after some time. When this happened, we stopped the computation after we did not see any new rule in output for more than 2 hours.
...
The second column shows the total number of predicates for which \amie produced at least one rule before getting stuck, while in brackets we report the total number of predicates.
...
The third and fourth columns report the total running time of the two approaches. Despite being disk-based, \krd can successfully complete the task faster than \amie in all cases, except for \yago 2. This is because of the very small size of the KB, which can easily fits in 
%main 
memory. However, when we deal with real KBs (\yago 3*), \amie is not even capable of loading the KB due to out of memory errors. Eventually the last column reports the total time needed to compute \texttt{rdf:type} information for each predicate in the KB. This time is negligible w.r.t. the total running time. 
...
Unfortunately, a KB can only provide positive examples. Hence in \deepdive the burden of creating negative examples is left to the user through manual rules definition.

In this set of experiments we will use our negative rules on \dbpedia to generate negative examples, and we will compare the output of \deepdive upon its showcase spouse example trained with different sets of negative examples.
...
Figure~\ref{fig:deep_dive_1k} shows \deepdive accuracy plot run on 1K input documents. The accuracy plot shows the fraction of correct positive predictions over total predictions (y-axis), for each output probability value (x-axis). It is evident that all the facts should be predicted with a probability of 1 by the perfect algorithm and zero facts should be predicted at an output probability of 0 which is the idealistic expected behavior marked by the dotted blue line. The best algorithm deflects the least from the blue dotted line and this serves as our evaluation metric.
...
In this last section we skecth the impact of individual components in \krd.
Full results on the internal evaluation are not reported in this paper due to space reasons but can be examined on the technical report online at \textbf{TODO: add online link to technical report}.

Here we briefly outline some of the most relevant findings. 
\begin{inparaenum}[\itshape 1)]
	\item We show the benefits of including literals in the mining especially for negative rules, where we double the number of errors discovered with an almost 10\% increase in precision.
	\item We show that $3$ is the optimal value for the $maxPathLen$ parameter, since with shorter rules we lose several meaningful rules and with longer ones we do not gain anything in precision or errors detected.
	\item Other than $maxPathLen$ parameter, we empirically validate the choices of $\alpha$ and $\beta$ for both positive and negative settings.
	...
	\item We quantify the benefits of $A^*$ search algorithm and pruning, where on an average the running time is halved.
	\item Eventually we show the benefits of our set cover problem formulation w.r.t. a ranking based one. 
	We show that the best rules need not necessarily be among the top-10, and sometimes there exist good rules that are beyond the 100$th$ one in the ranked set thus emphasizing upon the fragility in ranking mechanisms.
\end{inparaenum}
