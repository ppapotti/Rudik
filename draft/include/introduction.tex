...
We give a \texttt{log($k$)}-approximation algorithm for the discovery problem, % close in spirit to the greedy algorithm for set cover, 
...
materializes the KB as a graph, and discovers rules by navigating paths. To reduce the use of resources, at each iteration we follow the most promising path in a $A^*$ traversal fashion, allowing the pruning of unpromising paths.	
...
Also, \krd performs consistency with KBs of every size, while other systems cannot scale of fail altogether. % due clearly outperforming state-of-the-art systems . 
We also 
