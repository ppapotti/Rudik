...
In the following, parameters $\alpha$ and $\beta$ of Equation~\ref{eq:weight_fun} were set to  $\alpha=0.3$ and $\beta=0.7$ for positive rules, and to $\alpha=0.4$ and $\beta=0.6$ for negative rules. 
...
The full suite of test results, together with all the KBs, induced rules, and annotated gold standard examples and rules is available online at \url{http://bit.ly/2csROsc}.
...
Table~\ref{tab:vs_amie_neg} shows that \krd outperforms \amie in both cases with a precision gain of almost 20\%. The drop in quality for \krd w.r.t. the ones showed in Section~\ref{sec:gen_evaluation} is because we are using the \amie modified KBs which do not contain literals.
...
%main 
memory. However, when we deal with real KBs (\yago 3*), the KB could not even be loaded due to out of memory errors. The last column reports the total time needed to compute \texttt{rdf:type} information for each predicate in the KB. 
...
\begin{table}[htb]
	...
	\begin{scriptsize}
		\begin{tabular}{c|c|c|c|c|}
			...
			\hline
			\multicolumn{1}{ |c| }{\it Rules}&{\it Run Time} & {\it Precision} &{\it Run Time} & {\it Precision} \tabularnewline
			\hline
			\multicolumn{1}{ |c| }{+ve} & $\sim$35min & \textbf{63.99}\% & $\sim$54min & 60.49\%\tabularnewline
			\multicolumn{1}{ |c| }{-ve} &  $\sim$20min & \textbf{92.38}\% (499) &$\sim$25min &84.85\% (235) \tabularnewline
			\hline
		\end{tabular}
	\end{scriptsize}
...
The $maxPathLen$ parameter fixes the maximum number of atoms allowed in the body of a rule. Low values for $maxPathLen$ may exclude from the search space meaningful rules, while high values may exponentially increase the search space and consequently the running time. 
\begin{table}[htb]
	\centering
	...
	\begin{scriptsize}
		\begin{tabular}{c|c|c|c|c|}
			\cline{2-5}
			& \multicolumn{2}{c|}{$MaxPathLen$ = \textbf{2}} & \multicolumn{2}{c|}{$MaxPathLen$ = \textbf{3}} 
			\tabularnewline
			\hline
			\multicolumn{1}{ |c| }{\it Rules}&{\it Run Time} & {\it Precision} &{\it Run Time} & {\it Precision} 
			 \tabularnewline
			\hline
			\multicolumn{1}{ |c| }{+ve} & \textbf{$\sim$3min}& 49.17\% & $\sim$35min & 63.99\%  \tabularnewline
			\multicolumn{1}{ |c| }{-ve} & \textbf{$\sim$56sec} & 90\% (131) & $\sim$20min & \textbf{92.38}\% (499)\tabularnewline
			\hline
		\end{tabular}
		\begin{tabular}{|c|c|}
			\cline{1-2}
			\multicolumn{2}{|c|}{$MaxPathLen$ = \textbf{4}} 
				\tabularnewline
				\hline
				\multicolumn{1}{ |c| }{\it Run Time} & {\it Precision}
				\tabularnewline
				\hline
				\multicolumn{1}{ |c| }{$\sim>$24h} &  \textbf{66}\%  \tabularnewline
				\multicolumn{1}{ |c| }{$\sim>$24h} & 92.71\% (302) \tabularnewline
				\hline
		\end{tabular}
	\end{scriptsize}
\end{table}
