...
public class App 
{
	...
	public String parseRuleMineReq( rule_miner rm ) throws IOException
	{
		ConfigurationFacility confUser = new ConfigurationFacility();
		confUser.setUserParameters(rm);
		int sign;
		if(rm.isGenNegRules())
			sign =0;
		else
		...
		String kbName;
		String relNameVal = rm.getRelName();
		Set<String> relations = Sets.newHashSet();
//		String typeObject=rm.getTypeOfObject(), typeSubject = rm.getTypeOfSubject();
		
		String dbPrefix ;
		if(rm.getkBase().equals("dbpedia"))
			dbPrefix = "http://dbpedia.org/ontology/";
		else if(rm.getkBase().equals("yago"))
		...
		String typeObject=dbPrefix + rm.getTypeOfObject(), typeSubject = dbPrefix + rm.getTypeOfSubject();
		
		if ((rm.getkBase().equals("dbpedia")) && (relNameVal.equals("founder"))) {
			relations.add(dbPrefix + "founder");
			relations.add(dbPrefix + "foundedBy");
//			relations.add("http://dbpedia.org/ontology/foundedBy");
		}
		else {
			relations.add(dbPrefix + relNameVal);
//			relations.add("http://dbpedia.org/ontology/foundedBy");
		}
		int limitSubject = rm.getEdgeLimit();
		int limitObject = rm.getEdgeLimit();
		...
		int genLimit = -1, smartLimit = -1;
		int posLimit = -1, negLimit = -1;
		String id;
		
		if(rm.isUseSmartSampling()){
			smartLimit = rm.getGenLimit();
			double alpha = rm.getAlphaSmart(), beta =rm.getBetaSmart(), gamma =rm.getGammaSmart(), 
					subWeight = rm.getSubWeight(), objWeight =rm.getObjWeight();
			boolean isTopK = rm.isTopK();
			...
			id = relations+"_"+limitSubject+"_"+limitObject+"_smarLimit:"+smartLimit+"_alpha:"+alpha
		             +"_beta:"+beta+"_gamma:"+gamma+"_isTopK:"+isTopK;
			
		}
		else{
			genLimit = rm.getGenLimit();
			...
			id = relations+"_"+limitSubject+"_"+limitObject+"_posLimit:"+posLimit+"_negLimit:"+negLimit;
		}
		...
		String opResult = StatisticsContainer.printStatistics();
...
//
//
//			return "No rules generated in this execution !!!!!";
			}
